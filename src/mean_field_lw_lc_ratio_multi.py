...
np.savetxt("data/"+'time.txt', time, fmt='%.6f', delimiter=',')
# sim params
...
try:
    ...
    rand_index = np.random.randint(len(t0_list))
    t0 = t0_list[rand_index]
    k_coef = k_list[rand_index]
    np.savetxt("t0_k_coefs.csv", np.array([rand_index, t0, k_coef]), fmt='%.6f', delimiter=',')

Delta_c_shape = np.exp(-k_coef*(time-t0)) / ( 1+np.exp(-k_coef*(time-t0)) )**2
# delta_w = b_w * time
delta_w = b_w * (1 - np.exp(-time/tau_w))
delta_c = b_c * Delta_c_shape
# temporal delta functions
...
for org_c in range(n_org):
    ...
    # np.savetxt("delta_w_guess_0.txt",   delta_w,   fmt='%.6f', delimiter=',')
    # np.savetxt("delta_c_guess_0.txt",   delta_c,   fmt='%.6f', delimiter=',')
    # read delta_w, delta_c
    ...
    if PbyP_switch: # propagation by proliferation happens. l_w and l_c might change.
        ...
        for dt_c in range(0, n_time_steps):
            ...
            # beta_w_aff = beta_w_unaff * np.exp(-delta_w[dt_c] * (1/a_c) * (A_c_v[dt_c])) # this was the previous one (beta_term=0)
            beta_w_aff = beta_w_term + (beta_w_unaff -beta_w_term)* np.exp(-delta_w[dt_c] * (1/a_c) * (A_c_v[dt_c]))
...
# np.savetxt("data/"+'r.txt', r, fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'th_1.txt', th_1, fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'th_2.txt', th_2, fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'th_3.txt', th_3, fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'th_v_w.txt', th_v_w, fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'th_v_c.txt', th_v_c, fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'A_w.txt', A_w, fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'A_c.txt', A_c, fmt='%.6f', delimiter=',')

# # np.savetxt("data/"+'G.txt', G_int, fmt='%.6f', delimiter=',')
# # np.savetxt("data/"+'F.txt', F_int, fmt='%.6f', delimiter=',')

# # np.savetxt("data/"+'beta_w_ll.txt', beta_w_ll, fmt='%.6f', delimiter=',')
# # np.savetxt("data/"+'beta_c_ll.txt', beta_c_ll, fmt='%.6f', delimiter=',')

# np.savetxt("data/"+'beta_w_aff_list.txt', beta_w_aff_list, fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'beta_c_aff_list.txt', beta_c_aff_list, fmt='%.6f', delimiter=',')

# np.savetxt("data/"+'A_w_aff.txt',   A_w_aff,   fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'A_w_unaff.txt', A_w_unaff, fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'A_c_aff.txt',   A_c_aff,   fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'A_c_unaff.txt', A_c_unaff, fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'A_c_v.txt',   A_c_v,   fmt='%.6f', delimiter=',')
# np.savetxt("data/"+'A_w_v.txt',   A_w_v, fmt='%.6f', delimiter=',')

# # np.savetxt("data/"+'l_w_list.txt', l_w_list, fmt='%.6f', delimiter=',')
# # np.savetxt("data/"+'l_c_list.txt', l_c_list, fmt='%.6f', delimiter=',')
